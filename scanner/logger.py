...
def get_logger() -> logging.Logger:
    ...
    logger = logging.getLogger('fim-scanner')
...
def write_daily_checksum() -> str:
    ...
    path = Path(LOG_DIR) / 'scanner.log'
    if path.exists():
        digest = hashlib.sha256(path.read_bytes()).hexdigest()
    else:
        digest = hashlib.sha256(b'').hexdigest()
    out = Path(LOG_DIR) / time.strftime('scanner-%Y-%m-%d.sha256')
    out.write_text(digest + '  scanner.log\n', encoding='utf-8')
    return digest
