...
def baseline_exists() -> bool:
    ...
    with conn_cursor() as cur:
        cur.execute('SELECT EXISTS(SELECT 1 FROM baseline_hashes) AS ok')
...
def monitored_paths() -> List[str]:
    ...
    with conn_cursor() as cur:
        cur.execute('SELECT file_path FROM monitored_files WHERE is_active=TRUE ORDER BY file_path')
...
def severities() -> Dict[str, str]:
    ...
    with conn_cursor() as cur:
        cur.execute('SELECT file_path,severity FROM monitored_files')
        return {r['file_path']: r['severity'] for r in cur.fetchall()}


def start_scan(triggered_by: str) -> int:
    """Create a scan run."""
    with conn_cursor() as cur:
        cur.execute('INSERT INTO scan_runs(triggered_by) VALUES(%s) RETURNING id', (triggered_by,))
...
def finish_scan(scan_id: int, stats: Dict[str, int], status: str) -> None:
    ...
    with conn_cursor() as cur:
        cur.execute(
            'UPDATE scan_runs SET completed_at=NOW(),duration_ms=%s,files_scanned=%s,files_clean=%s,files_modified=%s,files_deleted=%s,files_added=%s,status=%s WHERE id=%s',
            (
...
def replace_baseline(scan_id: int, snaps: Iterable[Dict[str, object]]) -> None:
    ...
    with conn_cursor() as cur:
        ...
        for s in snaps:
            cur.execute(
                'INSERT INTO baseline_hashes(file_path,sha256_hash,file_size,permissions,owner_uid,owner_gid,owner_name,inode,baseline_scan_id) '
...
def load_baseline() -> Dict[str, Dict[str, object]]:
    ...
    with conn_cursor() as cur:
        cur.execute('SELECT * FROM baseline_hashes')
        return {r['file_path']: dict(r) for r in cur.fetchall()}
...
def write_event(scan_id: int, ev: Dict[str, object]) -> None:
    ...
    with conn_cursor() as cur:
        cur.execute(
            'INSERT INTO file_events(scan_run_id,file_path,event_type,severity,hash_before,hash_after,size_before,size_after,permissions_before,permissions_after,owner_before,owner_after) '
