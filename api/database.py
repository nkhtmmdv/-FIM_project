...
def init_pool()->None:
    ...
    global DB_POOL
    DB_POOL=pool.SimpleConnectionPool(1,20,host=os.getenv('POSTGRES_HOST','postgres'),dbname=os.getenv('POSTGRES_DB','fim'),user=os.getenv('POSTGRES_USER','fim_app'),password=os.getenv('POSTGRES_PASSWORD',''))
@contextmanager
def cursor():
...
def fetch_all(sql:str, params:Sequence[Any]=())->List[Dict[str,Any]]:
    ...
    with cursor() as cur: cur.execute(sql,params); return [dict(r) for r in cur.fetchall()]
def fetch_one(sql:str, params:Sequence[Any]=())->Optional[Dict[str,Any]]:
    """Fetch one row for parameterised SQL."""
    with cursor() as cur: cur.execute(sql,params); row=cur.fetchone(); return dict(row) if row else None
def execute(sql:str, params:Sequence[Any]=())->None:
    """Execute parameterised SQL."""
    with cursor() as cur: cur.execute(sql,params)
def audit(username:str, action:str, target:str|None, ip:str|None)->None:
    """Write an audit log entry."""
    execute('INSERT INTO audit_log(username,action,target,ip_address) VALUES(%s,%s,%s,%s)',(username,action,target,ip))
