...
def _s(row:dict)->dict:
    if row and 'file_path' in row: row=dict(row); p=row['file_path']; row['file_path']=p[len(_ROOT):] if p and p.startswith(_ROOT+'/') else p
    return row
@router.get('')
def alerts(user=Depends(current_user),severity:str|None=None,event_type:str|None=None,path:str|None=None,scan_run_id:int|None=None,limit:int=Query(50,le=500),offset:int=0):
    ...
    params.extend([limit,offset]); sql='SELECT * FROM file_events WHERE '+ ' AND '.join(where)+' ORDER BY detected_at DESC LIMIT %s OFFSET %s'; return [_s(r) for r in fetch_all(sql,params)]
@router.get('/recent')
