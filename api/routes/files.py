...
def _strip(row:dict)->dict:
    ...
    if row and 'file_path' in row: row=dict(row); row['file_path']=_to_display(row['file_path'])
    return row
@router.get('')
def list_files(user=Depends(current_user), status:str|None=None, severity:str|None=None, limit:int=Query(50,le=200), offset:int=0):
    ...
    params.extend([limit,offset]); return [_strip(r) for r in fetch_all(sql, params)]
@router.get('/{path:path}')
def file_detail(path:str,user=Depends(current_user)):
    """Return a single file and history."""
    scan_path=_to_scan('/'+path); f=fetch_one('SELECT * FROM monitored_files WHERE file_path=%s',(scan_path,)); hist=fetch_all('SELECT * FROM file_events WHERE file_path=%s ORDER BY detected_at DESC LIMIT 200',(scan_path,)); return {'file':_strip(f),'history':[_strip(h) for h in hist]}
@router.post('/add')
def add_file(body:FileAdd,request:Request,user=Depends(current_user)):
    ...
    execute('INSERT INTO monitored_files(file_path,severity,added_by) VALUES(%s,%s,%s) ON CONFLICT(file_path) DO UPDATE SET is_active=TRUE,severity=EXCLUDED.severity',(scan_path,body.severity,user['username'])); audit(user['username'],'file.add',scan_path,request.client.host if request.client else None); return {'ok':True}
@router.delete('/{path:path}')
def remove_file(path:str,request:Request,user=Depends(current_user)):
    """Deactivate a monitored file path."""
    target=_to_scan('/'+path); execute('UPDATE monitored_files SET is_active=FALSE WHERE file_path=%s',(target,)); audit(user['username'],'file.remove',target,request.client.host if request.client else None); return {'ok':True}
...
def enable_file(path:str,request:Request,user=Depends(current_user)):
    ...
    target=_to_scan('/'+path); execute('UPDATE monitored_files SET is_active=TRUE WHERE file_path=%s',(target,)); audit(user['username'],'file.enable',target,request.client.host if request.client else None); return {'ok':True}
